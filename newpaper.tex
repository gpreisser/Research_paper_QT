...
\caption{Probability distribution of spontaneous emissions for
  ...
  \kappa = \gamma = g = 1$, $100,000$ quantum trajectories. } \label{probdiss}
\end{figure}
...
we count the number of spontaneus photons until the moment where the
...
thus the time each quantum trajectory is evolved is not fixed. Second,
we fix the time each quantum trajectory is evolved and count the
number of spontaneus emissions $n_s$, and cavity emissions $n_c$. From
the set of quantum trajectories evolved a fixed time, we calculate the
joint probability $p(n_s,n_c)$, of finding $n_s$ spontaneous emissions
and $n_c$ cavity emissions. Using $p(n_s,n_c)$ we calculate the
correlations between the two random variables. This correlation is a
measure of how much information we can obtain about $n_s$ if we know
$n_c$.
...
results are shown in Fig.~\ref{probdiss}. As the number of leaked
...
the time we measure spontaneous emitted photons, which implies a
larger mean number of emitted photons. Also, as shown in
Fig~\ref{graph}, the probability distribution is wider as the number
of leaked photons increases, this implies less accuracy guessing the
number of spontaneus emission. If we know the cavity drive, this
results does not imply that we can learn something extra about the
number of spontaneous emissions knowing the number of photons leaked
from the cavity; it is clear that when we increase the time window
where we count photons, both, the number of leaked photons and the
number of spontaneous emissions, increases.
...
\begin{figure}[!h]  
\centering
\includegraphics[scale = 0.5]{newsigmaeng.pdf}
\caption{Average number of spontaneous emissions $\pm \sigma$ with respect to cavity emissions, under parameters $\expval{n} \approx 1$, $\gamma = 1$ , $\kappa = g = 0.1$ and $\mathcal{E} =  \kappa |\langle n \rangle|[1 + 2g^2/\gamma \kappa]$.}
\label{graph}
\end{figure} 
...
corresponds to fixing the time we measure photons. Each set of quantum
trajectories with the same duration is labelled with its mean number
of spontaneous emitted photons $\mu$. We consider the case where we
fix the mean number of photons inside the cavity to $1$. As shown by
...
emitted photons is shown for different mean values. As it is shown in
Fig.~\ref{error2}(b), the width of the distribution increases linearly
with the mean number of spontaneus emission. This behaviour does not
imply any correlation between the number of spontaneus emissions and
cavity emissions.
...
$p(n_s|n_c)$, the probability distribution of the number of spontaneus
...
In Fig.~\ref{probdisult} (a) we show the probability distribution of
$n_s$ for the case where $n_c$.



In Fig.~\ref{probdisult} (b), where we obtain a variance almost equal
to the mean value, something which is characteristic of Poisson
distributions. 
...
\caption{(a) Probability distributions for different mean number of
  spontaneous emissions $\mu$; (b) Calculation of variance with
  respect to the mean number of sponatenous emissions $\mu$. In both
  cases we consider $\expval{n} \approx 1$, $\gamma = 1.0$ ,
  $\kappa = g = 0.1$ and
  $\mathcal{E} = \kappa |\langle n \rangle|[1 + 2g^2/\gamma
  \kappa]$.} \label{error2}
\end{figure*}
\end{center}
